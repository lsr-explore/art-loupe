...
| 12 | Routing call — deterministic face gate feeds a structured-output manifest and declination reason; checksum-keyed node cache | 12a done (#65): deterministic path, cache, FR-801 check. 12b pending: the model-driven Director ([`routing-plan.md`](./routing-plan.md)) | Keeps the critical path deterministic where it can be |
