...
| 12 | Routing call — deterministic face gate feeds a structured-output manifest and declination reason; checksum-keyed node cache | Done (#65, #67): 12a is the deterministic path, the cache and the FR-801 check; 12b is the model-driven Director ([`routing-plan.md`](./routing-plan.md)) | Keeps the critical path deterministic where it can be |
