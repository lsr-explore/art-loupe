...
/**
 * Roles allowed into this app. The dashboard is an operator surface;
 * `superuser` retains full access. An artist session (e.g. a cross-app
 * cookie from the companion) is treated as unauthorized here so the apps stay separated.
 */
const ALLOWED_ROLES = new Set<Role>(['operator', 'superuser']);
