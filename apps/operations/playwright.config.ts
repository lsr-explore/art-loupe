...
export default defineConfig({
  ...
  use: {
     ...
     *
     * This app's middleware bounces an unacknowledged visitor to the entry point on
     * **every** route, including the login screen — so without this seed the whole
     * suite redirects to :3003, which this config never starts. Seeding the cookie
     * is not a bypass: it is the state of every real visitor, who reached this app
     * by ticking the box on the apex.
     *
  ...
  webServer: {
    ...
    // the proxy matcher excludes it and it is served directly.
