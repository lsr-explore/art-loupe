"""Running a graph under its guards.
...
These run small graphs of their own rather than the studio graph. The studio graph reads the
artist's data, and none of this is about that; `test_graph.py` runs the real one.
"""

import pytest
from agent_support import FakeArtistApi
from langgraph.graph import END, START, StateGraph

from artloupe.agent.resources import RunResources, run_resources
...
def _single(name: str, node) -> object:
...
def seed_node(state: RunState) -> dict[str, list[str]]:
...
def initial(run_id: str = "run-1") -> RunState:
    return {"run_id": run_id, "owner": "artist-1", "project_id": "project-1", "node_trail": []}
...
async def test_a_graph_invoked_without_a_recorder_still_runs() -> None:
    ...
    result = await _single("seed", seed_node).ainvoke(initial())
    assert result["node_trail"] == ["seed:run-1"]


async def test_run_resources_reach_every_node_and_leave_with_the_run() -> None:
    """The token-bearing client is visible inside the run and nowhere after it."""
    resources = RunResources(api=FakeArtistApi())  # type: ignore[arg-type]
    seen: list[RunResources] = []

    def reads_resources(_state: RunState) -> dict[str, list[str]]:
        seen.append(run_resources())
        return {"node_trail": ["read"]}

    await execute_run(
        _single("read", reads_resources),
        initial(),
        run_id="run-1",
        owner="artist-1",
        guards=GENEROUS,
